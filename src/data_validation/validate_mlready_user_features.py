...
def build_expectations():
    """
    Define all expectations for mlready.user_features.
    """
    return [
        # Check if reviewer_id exists and has no missing values
        gx.expectations.ExpectColumnValuesToNotBeNull(
            column="reviewer_id",
            severity="critical",
        ),

        # Ensure reviewer_id is unique across the dataset
        ...
        gx.expectations.ExpectColumnToExist(column="distinct_products_count"),
        gx.expectations.ExpectColumnToExist(column="avg_rating"),
        gx.expectations.ExpectColumnToExist(column="min_rating"),
        gx.expectations.ExpectColumnToExist(column="max_rating"),
        ...
        gx.expectations.ExpectColumnToExist(column="active_days_span"),
        ...
        # Count-based metrics must be non-negative
        ...
        gx.expectations.ExpectColumnValuesToBeBetween(
            column="distinct_products_count",
            min_value=1,
            severity="critical",
        ),
        gx.expectations.ExpectColumnValuesToBeBetween(
        ...
        gx.expectations.ExpectColumnValuesToBeBetween(
            column="active_days_span",
            min_value=0,
            severity="critical",
        ),

        # Rating-related metrics should stay within the expected domain
        gx.expectations.ExpectColumnValuesToBeBetween(
            column="avg_rating",
            min_value=1,
            max_value=5,
...
# Airflow entrypoint
def run_mlready_user_features_validation(
    execution_date: str,
    output_dir: str = "src/data_validation/gx_results",
) -> None:
    """
    Main entrypoint for Airflow.
    Runs Great Expectations validation for mlready.user_features.
    """
    table_name = iceberg_cfg["table_users"]
    spark = create_spark_session("validate-mlready-user-features")

    logger.info("Starting validation for mlready.user_features")
    ...
    try:
        ...
        if not success:
            raise ValueError("Validation failed for mlready.user_features")

    except Exception as exc:
        logger.exception(f"Validation failed for mlready.user_features: {exc}")
        raise
